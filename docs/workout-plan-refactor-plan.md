...
3. **Idempotent transform.** Group legacy `weeks[].sessions[]` items by workout date, then convert each date group into one completed workout in `users/{uid}/workouts/{deterministicId}`. Each legacy exercise log on that date becomes one `WorkoutExercise` inside the grouped workout. Re-running the migration overwrites the same migrated documents instead of duplicating them. Legacy sets should be migrated as `setType: "working"`; warm-up sets only appear in newly logged v2 workouts unless an old field explicitly identifies them.
...
6. **Represent legacy sessions safely.** Because each legacy entry is one exercise log, migrate it into a traceable `WorkoutExercise` within the date-grouped workout. Left/right legacy sessions become two blocks (`left`, `right`); bilateral sessions become one `both` block. Target reps are unknown, so mark target reps as empty/legacy-derived and preserve actual reps, raw values, notes, original IDs, week metadata, and raw payloads for verification.
   - Each migrated `WorkoutExercise.legacy.sourceTraceKey` must contain exactly the stable source coordinates `{ legacyWeekIndex, legacySessionIndex, legacySessionId, originalExerciseId, date }`.
   - Each migrated `WorkoutExercise.legacy.sourceTraceId` must equal the deterministic stable hash of the canonical `sourceTraceKey`; validation must compare canonical trace-key objects/serialized keys and must not trust a supplied trace ID by itself.
   - Preview validation must prove every normalized source trace key appears exactly once in transformed output, every transformed key exists in the normalized source, and no transformed workout exercises duplicate or omit a source trace key.
...
11. **Deep migration verification:** Compare expected and read-back migrated documents using canonical transformed-data fingerprints, exact expected document IDs, workout dates, exercise/block/set structure, raw reps, raw weights, side, set number, completion status, and legacy trace metadata.
12. **Changed-source guard:** Before execution writes v2 data, read `users/{uid}/settings/main`. If it already says `migrationStatus: "completed"`, both `sourceFingerprint` and `legacySourceFingerprint` must be present, must match each other, and must match the newly previewed legacy source fingerprint. Missing metadata, internally inconsistent metadata, or metadata for a different legacy source blocks execution before writing exercises, workouts, or running settings. This avoids leaving stale migrated documents from two different legacy generations in the same deterministic collections. The migration runner does not automatically delete, repair, or replace stale generations because deletion or metadata repair would be a destructive operation that needs explicit review after verification. A matching completed fingerprint may be safely re-run idempotently. A previous `failed` state may be retried deliberately; a previous `running` state blocks until manually reviewed because it may represent a partial in-flight or interrupted migration.

### Phase 4 completed plan-management design

Phase 4 adds plan management only. It does not start workouts from plans, does not switch legacy logging/history/statistics to v2, and does not run the migration.

**Firestore paths**

- Plans: `users/{uid}/plans/{planId}`
- Reusable exercise definitions: `users/{uid}/exercises/{exerciseId}`
- No Phase 4 code writes to `rehabData/{uid}`.

**Final plan document shape**
...
type PlanExercise = {
  ...
  exerciseId: string;
  exerciseNameSnapshot: string;
  exerciseType: "strength" | "timed_hold" | "cardio" | "mobility" | "foam_rolling";
  sortOrder: number;
  prescription: StrengthPrescription | TimedHoldPrescription | CardioPrescription | ListPrescription;
  notes: string;
};
```

**Reusable exercise definition additions**

Exercise definitions still use the v2 `users/{uid}/exercises/{exerciseId}` collection. Phase 4 adds `exerciseType` and `trackingType` so plans do not force all exercises into strength-style sets × reps × weight. Existing migrated exercise metadata remains compatible and must not be removed.

**Type-specific prescriptions**
...
type StrengthPrescription = {
  blocks: Array<{
    ...
    side: "both" | "left" | "right";
    targetSets: number;
    targetReps: { type: "fixed"; value: number } | { type: "range"; min: number; max: number };
    targetWeight: { mode: "previous" } | { mode: "manual"; value: number } | { mode: "none" };
    unit: "kg" | "lb";
    restSeconds?: number;
    notes: string;
    sortOrder: number;
...
type ListPrescription = {
  items: Array<{ id: string; name: string; sortOrder: number }>;
  notes: string;
};
```

Mobility and foam rolling both use `ListPrescription`; the UI labels items as stretches or areas. Phase 5 will decide how these are completed during workout logging.

**Version policy**

- New plans start at version `1`.
- Duplicated plans start at version `1` and store `sourcePlanId`.
- Saving meaningful plan content changes increments version.
- Meaningful content includes plan name/description, sessions, session notes/order, plan exercises, prescription data, and exercise notes/order. Status fields such as active/archive state are saved but do not increment version by themselves.
- Persistence metadata such as `createdAt`, `updatedAt`, and `updatedAtToken` does not increment version by itself.
- Activating, deactivating, archiving, or restoring a plan updates metadata/status but does not create a separate historical plan copy.
- Completed workouts will later store snapshots, so editing a plan must not mutate completed workout documents.

**Archive and restore behavior**

Plans and exercise definitions are never permanently deleted in Phase 4. Archiving sets status metadata and keeps the document readable. Restoring an archived plan returns it as inactive/draft. Archived exercise definitions stay visible to existing plan snapshots and can be included in the picker with an explicit archived option.

**Concurrency/conflict behavior**

Plan updates use a simple `updatedAtToken` read-before-write transaction. The editor records the token loaded with the plan; saving fails with a clear conflict if the stored token changed before save. Create/save buttons also track saving state to avoid accidental duplicate submissions. This is intentionally simpler than enterprise revision history while still protecting against obvious stale overwrites.

**Phase 4 stop point**

The Plans UI can create, edit, duplicate, activate/deactivate, archive/restore plans, manage reusable exercise definitions, and edit reusable sessions/exercises/prescriptions. It does not start workouts, create workout logs, read history from v2, execute migration, or change legacy `rehabData/{uid}` behavior.
...
- **Changes:** Add an authenticated migration service with separate preview and explicit execution operations. Preview reads legacy data, transforms date-grouped workouts, fingerprints the normalized source, fingerprints the transformed v2 output, validates exact canonical source trace keys plus their matching `sourceTraceId` hashes, and reports counts without writes. Execution re-reads legacy data, verifies the source fingerprint, checks existing migration settings for a completed different-source migration before any v2 writes, writes v2 collections with deterministic IDs, reads back only the expected migrated document IDs, performs deep canonical verification, records migration metadata, and can be re-run safely when the source fingerprint matches.
- **Files:** `src/lib/firebase/migrations.*`, `src/lib/firebase/v2Repository.*`, `src/migrations/*`, optional admin/dev-only UI in settings.
- **Why:** Existing history must be preserved before the UI switches schemas.
- **Risks:** Firestore permissions, partial writes, or network failures; use conservative 450-operation batches, report committed batch counts on partial failure, atomically finalise successful settings/run metadata in one batch, and mark settings as failed when execution fails after writes have started.
- **Date policy:** Validate workout dates strictly as `YYYY-MM-DD` calendar dates. Blank or invalid legacy dates are warnings, not blocking errors, so legacy history can still be migrated and reviewed without data loss.
- **Prior migration state policy:** A completed migration can be re-run only when both stored fingerprints are present, agree with each other, and agree with the preview fingerprint; completed metadata that is missing, internally inconsistent, or from a different source is blocked before writes. Failed migrations can be retried deliberately and will overwrite deterministic v2 documents without deleting anything. Running migrations are blocked until manually reviewed or marked failed because they may indicate a partial write.
...
### Future workout date behavior

Approved future workout/history phases should support editing the workout date before save and later from workout history. Internally store workout dates as `YYYY-MM-DD`, use workout date rather than creation timestamp for graphs/PBs/progress, and display dates as `DD-MM-YY` such as `18-07-26`.
