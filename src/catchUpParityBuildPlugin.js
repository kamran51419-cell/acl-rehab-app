...
function transformWorkoutScreen(code, id) {
  ...
  next = replaceRequired(
    ...
    '<Button variant="outline" onClick={onClose}>Cancel</Button></div></section>{picker ? <WorkoutExercisePicker exercises={exerciseLibrary} replacing={null} onCancel={() => setPicker(false)} onChoose={addExerciseToDraft}/> : null}{editing ? <WorkoutExerciseEditor workout={draft} exercise={editing} exercises={exerciseLibrary} onCancel={() => setEditing(null)} onSave={(edited) => { setDraft((current) => ({ ...current, exercises: editWorkoutExerciseList(current, editing.id, edited, completedWorkouts) })); setEditing(null); }}/> : null}</div>;',
    id,
  )
