...
export function TodayRoutine({ groups, expanded = false, onToggle, onStatus }) {
  ...
  const visibleDue = expanded ? groups.due : groups.due.slice(0, VISIBLE_TASKS);
  const visibleOverdue = expanded ? groups.overdue : groups.overdue.slice(0, VISIBLE_TASKS);
  const hiddenCount = count - (visibleDue.length + visibleOverdue.length + groups.done.length);
  return <section aria-labelledby="today-routine-title" className="space-y-4"><div className="flex items-center justify-between"><h2 id="today-routine-title" className="text-base font-semibold">Routine</h2>{hiddenCount > 0 || expanded ? <button type="button" className="text-sm font-medium text-slate-600" onClick={onToggle}>{expanded ? "Show less" : `Show all (${count})`}</button> : null}</div>{!count ? <p className="text-sm text-slate-500">No routine tasks due today or overdue.</p> : null}{visibleDue.length ? <div className="space-y-2"><h3 className="text-sm font-semibold text-slate-700">Due Today</h3><div className="space-y-1.5">{visibleDue.map((task) => <TaskRow key={taskKey(task)} task={task} onStatus={onStatus}/>)}</div></div> : null}{visibleOverdue.length ? <div className="space-y-2"><h3 className="text-sm font-semibold text-orange-700">Overdue</h3><div className="space-y-1.5">{visibleOverdue.map((task) => <TaskRow key={taskKey(task)} task={task} overdue onStatus={onStatus}/>)}</div></div> : null}<CompletedTasks tasks={groups.done}/></section>;
}
...
export function HomeDashboard({ programme, unfinishedWorkout, incompleteWorkoutList, incompleteExpanded, surgeryDate, trainingMode = "gym", today, showSessions, onStart, onContinue, onContinueIncomplete, onDismissIncomplete, onToggleIncomplete, onChooseSession, onOneOff, routineGroups: groups, routineExpanded = false, onToggleRoutine, onRoutineStatus }) {
  ...
  return <div className="mx-auto max-w-2xl space-y-5"><TodayRoutine groups={groups} expanded={routineExpanded} onToggle={onToggleRoutine} onStatus={onRoutineStatus}/><IncompleteWorkouts workouts={incompleteWorkoutList} expanded={incompleteExpanded} onToggle={onToggleIncomplete} onContinue={onContinueIncomplete} onDismiss={onDismissIncomplete}/><section className="space-y-4"><Button className={`w-full py-3 text-base ${unfinishedWorkout ? "-translate-y-[12px] md:translate-y-0" : ""}`} disabled={!unfinishedWorkout && !programme} onClick={unfinishedWorkout ? onContinue : onStart}>{unfinishedWorkout ? "Continue Workout" : "Start Workout"}</Button>{!unfinishedWorkout ? <>{showSessions && programme ? <div className="workout-session-chooser rounded-2xl border border-slate-200 bg-white p-4 shadow-sm"><h2 className="font-semibold">Choose a session</h2><div className="mt-3 space-y-2">{sessions.map((session) => <button type="button" key={session.id} onClick={() => onChooseSession(session.id)} className="w-full rounded-xl border border-slate-200 px-4 py-3 text-left font-medium hover:bg-slate-50">{session.name}</button>)}<button type="button" onClick={onOneOff} className="quick-workout-option w-full rounded-xl border border-blue-200 bg-blue-50 px-4 py-3 text-left font-semibold text-blue-900 hover:bg-blue-100">Quick Workout</button></div></div> : null}{!showSessions ? <Button className="w-full py-3 text-base" variant="outline" onClick={onOneOff}>Quick Workout</Button> : null}</> : null}</section>{timeline ? <><div className="border-t border-slate-200"/><section><h2 className="text-sm font-medium text-slate-500">Rehab timeline</h2><div className="mt-3 grid grid-cols-3 gap-2">{Object.values(timeline).map((label) => <div key={label} className="rounded-xl bg-white p-3 text-center text-sm font-medium shadow-sm">{label}</div>)}</div></section></> : null}</div>;
}
...
export default function HomeScreen({ user, surgeryDate, trainingMode = "gym", onOpenWorkout, fromProgramme = false, onBackToProgramme, repository = defaultRepository }) {
  const [plans, setPlans] = useState([]); const [workouts, setWorkouts] = useState([]); const [occurrences, setOccurrences] = useState([]); const [showSessions, setShowSessions] = useState(false); const [incompleteExpanded, setIncompleteExpanded] = useState(false); const [routineExpanded, setRoutineExpanded] = useState(false);
  ...
  async function setStatus(task, status) {
    ...
    try {
      await repository.setRoutineOccurrenceStatus(db, user.uid, optimistic);
    } catch (error) {
