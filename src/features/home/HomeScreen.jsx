...
export function RecentWorkouts({ workouts }) {
  const completed = completedWorkoutHistory(workouts).slice(0, 12);
  if (!completed.length) return null;
  return <section className="space-y-3"><h2 className="text-lg font-semibold">Recent workouts</h2><div className="flex snap-x gap-3 overflow-x-auto pb-2">{completed.map((workout) => <article key={workout.id} className="min-w-[78%] snap-start rounded-2xl border border-slate-200 bg-white p-4 shadow-sm sm:min-w-[46%] lg:min-w-[22%]"><div className="font-semibold">{workout.sessionNameSnapshot || "Workout"}</div><div className="mt-1 text-sm text-slate-500">{formatDate(workout.date || workout.workoutDate)}</div><div className="mt-3 text-sm text-slate-600">{workout.exercises?.length || 0} exercises</div></article>)}</div></section>;
}
