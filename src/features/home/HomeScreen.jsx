...
export default function HomeScreen({ user, surgeryDate, onOpenWorkout, onOpenProgramme, fromProgramme = false, onBackToProgramme, repository = defaultRepository }) {
  ...
  return <div className="space-y-10">{fromProgramme ? <Button variant="outline" onClick={onBackToProgramme}>← Back to programme</Button> : null}<HomeDashboard programme={programme} unfinishedWorkout={unfinishedWorkout} surgeryDate={surgeryDate} rehabAgeMode={rehabAgeMode} today={todayString()} showSessions={showSessions} onStart={() => setShowSessions(true)} onContinue={() => onOpenWorkout({ mode: "continue", workoutId: unfinishedWorkout.id })} onChooseSession={(sessionId) => onOpenWorkout({ mode: "session", sessionId })} onProgramme={onOpenProgramme} onCycleAge={cycleAge} /><section id="exercise-library" className="scroll-mt-6"><PlansScreen user={user} view="exercises" /></section></div>;
}
