...
import { deleteWorkoutDocument } from "../../lib/firebase/planRepository";

const defaultRepository = {
  deleteWorkoutDocument,
  subscribeWorkouts(database, uid, onNext, onError) {
    return onSnapshot(
      collection(database, "users", uid, "workouts"),
...
export function WorkoutHistoryView({ workouts, selectedId, loading = false, loadError = "", deletingId, deleteError, onSelect, onRequestDelete, onCancelDelete, onConfirmDelete }) {
...
export default function WorkoutHistoryScreen({ user, highlightId = null, repository = defaultRepository }) {
  ...
  useEffect(() => {
    setLoading(true);
    setLoadError("");
    return repository.subscribeWorkouts(db, user.uid, (nextWorkouts) => {
      setWorkouts(nextWorkouts);
      setLoading(false);
    }, (error) => {
      console.error("Could not load workout history", error);
      setLoadError(error?.message || "Could not load workout history");
      setLoading(false);
    });
  }, [repository, user.uid]);
  ...
  useEffect(() => {
    ...
    setSelectedId((current) => completed.some((workout) => workout.id === current) ? current : completed[0].id);
  }, [completed]);
  ...
  return <WorkoutHistoryView workouts={workouts} selectedId={selectedId} loading={loading} loadError={loadError} deletingId={deleteCandidate ? deleting ? "pending" : deleteCandidate.id : null} deleteError={deleteError} onSelect={setSelectedId} onRequestDelete={setDeleteCandidate} onCancelDelete={() => !deleting && setDeleteCandidate(null)} onConfirmDelete={confirmDelete} />;
}
