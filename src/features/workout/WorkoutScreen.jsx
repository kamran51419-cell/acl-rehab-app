...
export function ExerciseCard({ exercise, oneOff, onChange, onAddSet, onRemoveSet, onRemoveExercise, onMove, index, total, hideExerciseName = false }) {
  ...
  return (
    <section className={`rounded-2xl border border-slate-200 bg-white p-4 ${hideExerciseName ? "shadow-none" : ""}`}>
      ...
      {isSetTickExercise ? <div className="mt-3 space-y-2">{(exercise.recordedSets || []).map((set) => <div key={set.id} className="grid grid-cols-[3.5rem_minmax(0,1fr)_2rem] items-center gap-3 rounded-xl bg-slate-50 p-3"><span className="text-sm font-medium">Set {set.setNumber}</span>{isRepsOnly ? <RepsInput exercise={exercise} set={set} onChange={onChange} /> : <span className="text-sm font-medium text-slate-700">{prescribedDuration}</span>}<SetTick checked={Boolean(set.completed)} label={set.completed ? `Mark set ${set.setNumber} incomplete` : `Mark set ${set.setNumber} complete`} onClick={() => onChange(exercise.id, set.id, "setCompleted", !set.completed)} /></div>)}</div>
