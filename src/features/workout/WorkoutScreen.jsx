...
function cleanUnsupportedSideExercise(exercise) {
  ...
  return {
    ...exercise,
    id: String(exercise.id || "").replace(/-(left|right)$/, ""),
...
export function normalizeWorkoutForDisplay(saved) {
  ...
  ordered(saved.exercises).forEach((original) => {
    ...
    const key = supportsSides(exercise)
      ? exercise.id
      : `${exercise.exerciseId || exercise.exerciseNameSnapshot}:${exercise.id}`;
...
function programmeSummary(exercise) {
  ...
  if (standardSide && supportsSides(exercise)) return summary.replace(/\s+both$/i, "");
  return summary;
}
...
export function ExerciseCard({ exercise, oneOff, onChange, onAddSet, onRemoveSet, onRemoveExercise, onMove, index, total }) {
  ...
  function CompactProgrammeContent() {
    if (stages.length > 0) {
      return <div className="min-w-0 flex-1 space-y-1">{stages.map((stage, stageIndex) => <div key={stage.id || stageIndex} className="text-sm font-medium text-slate-700">{`${durationLabel(stage.durationSeconds, stage.durationUnit) || "—"} ${stage.phase === "rest" ? "rest" : "work"}`}</div>)}</div>;
    }
      ...
      {isProgrammeWorkout && !isWeighted && !isRepsOnly ? (
        ...
        <div className="mt-3 space-y-2">
          {(exercise.recordedSets || []).map((set) => (
            <div key={set.id} className={`grid items-end gap-2 rounded-xl bg-slate-50 p-3 ${isWeighted ? "grid-cols-[3.25rem_minmax(0,1fr)_minmax(0,1fr)]" : "grid-cols-[3.25rem_minmax(0,1fr)]"}`}>
              <span className="pb-2 text-sm font-medium">Set {set.setNumber}</span>
              {fields.reps ? <label className="min-w-0 text-xs font-medium">Reps{set.prescribedReps?.type === "range" ? <select aria-label={`${exercise.exerciseNameSnapshot} set ${set.setNumber} reps`} className="mt-1 h-10 w-full rounded-lg border border-slate-200 bg-white px-3 text-slate-900" value={currentRepValue(set)} onChange={(event) => onChange(exercise.id, set.id, "actualReps", event.target.value)}><option value="" disabled hidden>{set.prescribedReps.min}–{set.prescribedReps.max}</option>{Array.from({ length: Number(set.prescribedReps.max) - Number(set.prescribedReps.min) + 1 }, (_, itemIndex) => Number(set.prescribedReps.min) + itemIndex).map((rep) => <option key={rep} value={rep}>{rep}</option>)}</select> : <input aria-label={`${exercise.exerciseNameSnapshot} set ${set.setNumber} reps`} inputMode="numeric" className="mt-1 h-10 w-full rounded-lg border border-slate-200 bg-white px-3 text-slate-900" value={currentRepValue(set)} onFocus={(event) => event.currentTarget.select()} onChange={(event) => onChange(exercise.id, set.id, "actualReps", event.target.value)} />}</label> : null}
              {isWeighted ? <label className="min-w-0 text-xs font-medium">Weight (kg)<input aria-label={`${exercise.exerciseNameSnapshot} set ${set.setNumber} weight`} inputMode="decimal" className="mt-1 h-10 w-full rounded-lg border border-slate-200 bg-white px-3 text-slate-900" value={set.rawWeight ?? set.weight ?? ""} onFocus={(event) => event.currentTarget.select()} onChange={(event) => onChange(exercise.id, set.id, "weight", event.target.value)} /></label> : null}
...
export default function WorkoutScreen({ user, repository = defaultRepository, intent, onIntentHandled = noop, onFinished = noop, onDiscarded = noop }) {
  ...
  useEffect(() => repository.subscribePlans(db, user.uid, setPlans, noop), [repository, user.uid]);
  useEffect(() => repository.subscribeWorkouts(db, user.uid, setWorkouts, noop), [repository, user.uid]);
  useEffect(() => repository.subscribeExerciseDefinitions(db, user.uid, setLibrary, noop), [repository, user.uid]);

  const saver = useMemo(() => createDebouncedSaver((value) => repository.updateInProgressWorkoutDocument(db, user.uid, value), 500, setSaveStatus), [repository, user.uid]);
  useEffect(() => () => { saver.flush().catch(noop); }, [saver]);
  useEffect(() => { if (workout) saver.schedule(workout); }, [saver, workout]);
  ...
  const startProgramme = async (session) => {
    ...
    const next = createInProgressWorkout({ id: `workout-${makeId()}`, userId: user.uid, programme, session, date: todayString(), previousWeightsByExercise: previous, createdAt: new Date().toISOString() });
    await repository.createInProgressWorkoutDocument(db, user.uid, next);
    setWorkouts((all) => [...all, next]);
    openSaved(next);
  };

  const startOneOff = async (name, exercises) => {
    if (unfinished) return openSaved(unfinished);
    const next = createOneOffWorkout({ id: `workout-${makeId()}`, userId: user.uid, name, exercises, date: todayString() });
    await repository.createInProgressWorkoutDocument(db, user.uid, next);
    setWorkouts((all) => [...all, next]);
    openSaved(next);
  };
  ...
  useEffect(() => {
    if (!intent || handled.current === intent.token) return;
    if (intent.mode === "continue" && unfinished) {
      handled.current = intent.token;
      openSaved(unfinished);
    } else if (intent.mode === "one_off") {
      handled.current = intent.token;
      unfinished ? openSaved(unfinished) : setBuilder(true);
    } else if (intent.mode === "session" && programme) {
      const session = programme.sessions?.find((item) => item.id === intent.sessionId);
      if (session) {
        handled.current = intent.token;
        startProgramme(session);
      }
    }
  ...
  const finish = async () => {
    ...
    try {
      ...
      const latest = { ...workout, durationSeconds: Number.isNaN(started.getTime()) ? undefined : Math.max(0, Math.round((now - started) / 1000)) };
      const completed = await completeWorkout(latest, saver, (item) => repository.finishWorkoutDocument(db, user.uid, item));
      setWorkout(null);
      onFinished(completed);
    } catch {
      setFinishError("Could not complete workout. Please try again.");
    } finally {
      setFinishing(false);
    }
  };
