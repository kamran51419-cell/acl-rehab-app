...
function transformWorkoutScreen(code, id) {
  ...
  const catchUpMapStart = next.indexOf('{ordered(draft.exercises).map((exercise, index) => ', completedIndex)
  if (catchUpMapStart < 0) throw new Error(`Left/right workout grouping transform could not find catch-up exercise map in ${id}`)
  next = next.slice(0, catchUpMapStart) + '{ordered(draft.exercises).map((exercise, index) => isLinkedWorkoutRightSide(ordered(draft.exercises), exercise, index) ? null : ' + next.slice(catchUpMapStart + '{ordered(draft.exercises).map((exercise, index) => '.length)

  const catchUpCardStart = next.indexOf('<ExerciseCard exercise={exercise}', catchUpMapStart)
  if (catchUpCardStart < 0) throw new Error(`Left/right workout grouping transform could not find catch-up exercise card in ${id}`)
  next = next.slice(0, catchUpCardStart) + '<WorkoutExerciseDisplay list={ordered(draft.exercises)} exercise={exercise}' + next.slice(catchUpCardStart + '<ExerciseCard exercise={exercise}'.length)
