...
const TOUCH_HELPER = `function startLongPressReorder(event, handlers) {
  ...
  const onMove = (moveEvent) => {
    ...
    if (!touch) return;
    if (!active) {
      if (Math.hypot(touch.clientX - start.clientX, touch.clientY - start.clientY) > 10) cleanup();
      return;
    }
    moveEvent.preventDefault();
    lastElement = document.elementFromPoint(touch.clientX, touch.clientY);
...
function transformWorkoutScreen(code, id) {
  ...
  next = replaceRequired(
    ...
    '<AutoGrowTextarea className="mt-1 min-h-10 w-full resize-none overflow-hidden rounded-xl border border-slate-200 p-3" value={workout.notes || ""} onChange={(event) => onNotes(event.target.value)}/>',
    id,
  )
  return next
}
