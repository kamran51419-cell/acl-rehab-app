...
function transformQuickWorkoutBuilder(code) {
  ...
  if (!next.includes('previousRepsForExercise')) {
  ...
  if (!next.includes('function latestEquipmentForExercise(')) {
    next = replaceIfPresent(
      next,
...
function transformWorkoutScreen(code) {
  ...
  if (start >= 0 && end > start) {
    ...
    next = next.replace(
      /buildQuickWorkout\(\{([^{}]*?)date:\s*todayString\(\)(?!,\s*completedWorkouts)([^{}]*?)\}\)/,
      'buildQuickWorkout({$1date: todayString(), completedWorkouts$2})'
    );
  }

  if (!next.includes('completedWorkouts={completedWorkouts}')) {
    next = next.replace('<QuickWorkoutBuilder ', '<QuickWorkoutBuilder completedWorkouts={completedWorkouts} ');
  }
  ...
  if (unfinishedStart >= 0 && unfinishedEnd > unfinishedStart) {
    const unfinished = 'const unfinished = workouts.find((item) => item.status === "in_progress" && item.completed !== true && !item.completedAt && (item.exercises || []).some(exerciseAttempted) && item.id !== suppressedWorkoutId);';
    next = next.slice(0, unfinishedStart) + unfinished + next.slice(unfinishedEnd + 1);
  }

  if (!next.includes('workout.sourceType === "one_off" ? "p-0"')) {
    ...
    if (formStart >= 0 && formEnd > formStart) {
      const sectionStart = next.indexOf('<section className="', formStart);
      if (sectionStart >= 0 && sectionStart < formEnd) {
        const classStart = sectionStart + '<section className="'.length;
        const classEnd = next.indexOf('"', classStart);
        if (classEnd > classStart && classEnd < formEnd) {
          const normalClass = next.slice(classStart, classEnd);
          const replacement = '<section className={workout.sourceType === "one_off" ? "p-0" : "' + normalClass + '"}';
          next = next.slice(0, sectionStart) + replacement + next.slice(classEnd + 1);
        }
      }
    }
  }
