function transformWorkoutScreen(code) {
  ...
  // Measurement labels stay on the top-left border. Previous values use the
  // same floating-border treatment on the bottom-right so narrow mobile fields
  // never have to fit both labels across the same edge.
  next = next.replaceAll(
    '<div className="min-w-0"><RepsInput exercise={exercise} set={set} onChange={onChange}/></div>',
    '<div className="workout-field-shell min-w-0"><div className="workout-floating-field"><div className="workout-field-control"><RepsInput exercise={exercise} set={set} onChange={onChange}/></div></div><div className="workout-field-topline"><span className="workout-topline-segment workout-topline-start"/><span className="workout-border-label">Reps</span><span className="workout-topline-segment workout-topline-end"/></div>{set.previousReps !== undefined && set.previousReps !== "" ? <div className="workout-field-bottomline"><span className="workout-border-label workout-border-label-previous">Prev. {set.previousReps}</span></div> : null}</div>',
  )
  ...
  next = next.replaceAll(
    ...
    '<div className="workout-field-shell min-w-0"><div className="workout-floating-field"><div className="workout-field-control"><input inputMode="decimal" className="h-10 w-full rounded-lg border border-slate-200 bg-white px-3" value={set.rawWeight ?? set.weight ?? ""} onFocus={(event) => event.currentTarget.select()} onChange={(event) => onChange(exercise.id, set.id, "weight", event.target.value)}/></div></div><div className="workout-field-topline"><span className="workout-topline-segment workout-topline-start"/><span className="workout-border-label">Weight (kg)</span><span className="workout-topline-segment workout-topline-end"/></div>{set.previousWeight !== undefined && set.previousWeight !== "" ? <div className="workout-field-bottomline"><span className="workout-border-label workout-border-label-previous">Prev. {set.previousWeight}</span></div> : null}</div>',
  )
  ...
  next = next.replaceAll(
    ...
    'onChange={(event) => onChange(exercise.id, set.id, "durationSeconds", event.target.value === "" ? "" : Number(event.target.value) * durationScale)}/></div></div><div className="workout-field-topline"><span className="workout-topline-segment workout-topline-start"/><span className="workout-border-label">Time ({durationUnitLabel})</span><span className="workout-topline-segment workout-topline-end"/></div>{set.previousDurationSeconds !== undefined && set.previousDurationSeconds !== "" ? <div className="workout-field-bottomline"><span className="workout-border-label workout-border-label-previous">Prev. {Number(set.previousDurationSeconds) / durationScale}</span></div> : null}</div>',
  )
  ...
  next = next.replaceAll(
    ...
    'onChange={(event) => onChange(exercise.id, set.id, "durationSeconds", event.target.value === "" ? "" : Number(event.target.value) * durationScale)}/></div></div><div className="workout-field-topline"><span className="workout-topline-segment workout-topline-start"/><span className="workout-border-label">Time ({durationUnitLabel})</span><span className="workout-topline-segment workout-topline-end"/></div>{set.previousDurationSeconds !== undefined && set.previousDurationSeconds !== "" ? <div className="workout-field-bottomline"><span className="workout-border-label workout-border-label-previous">Prev. {Number(set.previousDurationSeconds) / durationScale}</span></div> : null}</div> : null}{fields.distance ?',
  )
  next = next.replaceAll(
  ...
  next = next.replaceAll(
    ...
    'onChange={(event) => onChange(exercise.id, set.id, "distance", event.target.value)}/></div></div><div className="workout-field-topline"><span className="workout-topline-segment workout-topline-start"/><span className="workout-border-label">Distance (km)</span><span className="workout-topline-segment workout-topline-end"/></div>{set.previousDistance !== undefined && set.previousDistance !== "" ? <div className="workout-field-bottomline"><span className="workout-border-label workout-border-label-previous">Prev. {set.previousDistance}</span></div> : null}</div> : null}</div>',
  )
