  ...
  return {
    ...
    transform(code, id) {
      ...
      return replaceOnce(
        ...
        '<div className="min-w-0 flex-1 text-sm font-medium">{stages.length ? <div className="space-y-1.5">{stages.map((stage, stageIndex) => <div key={stage.id || stageIndex} className="flex items-baseline gap-2"><span>{durationLabel(stage.durationSeconds, stage.durationUnit)}</span><span className="text-slate-500">{stage.label?.trim() || `${String(stage.phase || "").charAt(0).toUpperCase()}${String(stage.phase || "").slice(1)}`}</span></div>)}</div> : programmeSummary(exercise) || "Complete exercise"}</div>',
        id,
      )
    },
  }
