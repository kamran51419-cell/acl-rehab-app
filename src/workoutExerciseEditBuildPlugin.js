...
function transformPlansScreen(code, id) {
  ...
  next = replaceOnce(
    ...
    '{(showAllSides || trainingMode === "rehab") && (\n  <>\n    <option value={SIDE.LEFT}>Left only</option>\n    <option value={SIDE.RIGHT}>Right only</option>\n  </>\n)}',
    id,
  )
  return next
}
