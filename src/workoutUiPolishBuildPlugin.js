...
function transformWorkoutScreen(code, id) {
  ...
  next = replaceOnce(
    ...
    '</div><div id="workout-edit-footer" className="shrink-0 grid grid-cols-2 gap-3 bg-white px-4 pb-4 pt-3 sm:px-5 sm:pb-5 sm:pt-4"><button type="button" className="h-11 rounded-xl border border-slate-200 bg-white px-4 text-sm font-medium text-slate-700 hover:bg-slate-50" onClick={onCancel}>Cancel</button><button id="workout-edit-save" type="button" className="h-11 rounded-xl px-4 text-sm font-semibold" onClick={() => onSave(draft)}>Save changes</button></div>',
    id,
  )
  return next
}
