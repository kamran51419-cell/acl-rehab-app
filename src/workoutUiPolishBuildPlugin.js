...
function transformWorkoutScreen(code, id) {
  ...
  next = replaceOnce(
    ...
    '<div id="workout-edit-footer" className="sticky bottom-0 z-20 -mx-4 mt-5 grid grid-cols-2 gap-3 border-t border-slate-100 bg-white px-4 pb-4 pt-3 sm:-mx-5 sm:px-5 sm:pb-5"><button type="button" className="h-11 rounded-xl border border-slate-200 bg-white px-4 text-sm font-medium text-slate-700 hover:bg-slate-50" onClick={onCancel}>Cancel</button><button id="workout-edit-save" type="button" className="h-11 rounded-xl px-4 text-sm font-semibold" onClick={() => onSave(draft)}>Save changes</button></div>',
    id,
  )
  return next
}
