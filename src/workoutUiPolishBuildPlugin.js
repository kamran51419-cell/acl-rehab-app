...
function transformWorkoutScreen(code, id) {
  ...
  next = replaceOnce(
    ...
    '<div className="fixed inset-0 z-50 flex items-end justify-center overflow-y-auto bg-slate-900/50 px-3 pb-6 pt-4 sm:items-center sm:p-6"><section className="max-h-[calc(100dvh-2.5rem)] w-full max-w-xl overflow-y-auto overscroll-contain rounded-2xl bg-white px-4 pb-10 pt-5 shadow-xl sm:max-h-[88vh] sm:px-5 sm:pb-9 sm:pt-6"><div><h2 className="text-lg font-semibold">Edit exercise</h2><p className="mt-1 text-xs text-slate-500">Changes here apply to this workout only. Your Programme stays unchanged.</p></div>',
    id,
  )
  next = replaceOnce(
    next,
    ...
    '<div className="mt-7 grid grid-cols-2 gap-3 pb-4"><button type="button" className="h-11 rounded-xl border border-slate-200 bg-white px-4 text-sm font-medium text-slate-700 hover:bg-slate-50" onClick={onCancel}>Cancel</button><button id="workout-edit-save" type="button" className="h-11 rounded-xl px-4 text-sm font-semibold" onClick={() => onSave(draft)}>Save changes</button></div>',
    id,
  )
  return next
}
